...
class Lang
{
    ...
    public function get($name = null, array $vars = [], $range = '')
    {
        ...
        if ($this->config['allow_group'] && strpos($name, '.')) {
            list($name1, $name2) = explode('.', $name, 2);
            $value = isset($this->lang[$range][strtolower($name1)][$name2]) ? $this->lang[$range][strtolower($name1)][$name2] : $name;
        } else {
            $value = isset($this->lang[$range][strtolower($name)]) ? $this->lang[$range][strtolower($name)] : $name;
        }
