...
class Socket implements LogHandlerInterface
{
    ...
    public function save(array $log = [])
    {
        ...
        if ($this->config['debug']) {
            if ($this->app->exists('request')) {
                ...
                $currentUri = 'cmd:' . implode(' ', isset($_SERVER['argv']) ? $_SERVER['argv'] : []);
            }
    ...
    protected function getClientArg($name)
    {
        ...
        if (empty($this->clientArg)) {
            ...
            parse_str(isset($match[1]) ? $match[1] : '', $this->clientArg);
        }
    ...
    protected function send($host, $port, $message = '', $address = '/')
    {
        ...
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, isset($this->config['curl_opt'][CURLOPT_CONNECTTIMEOUT]) ? $this->config['curl_opt'][CURLOPT_CONNECTTIMEOUT] : 1);
        curl_setopt($ch, CURLOPT_TIMEOUT, isset($this->config['curl_opt'][CURLOPT_TIMEOUT]) ? $this->config['curl_opt'][CURLOPT_TIMEOUT] : 10);
