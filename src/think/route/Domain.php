...
class Domain extends RuleGroup
{
     ...
     */
    public function __construct(Route $router, $name = null, $rule = null, $lazy = false)
    {
        $this->router = $router;
        $this->domain = $name;
        $this->rule   = $rule;
     ...
     */
    public function check(Request $request, $url, $completeMatch = false)
    {
        // 检测URL绑定
     ...
     * @return Dispatch|false
     */
    private function checkUrlBind(Request $request, $url)
    {
     ...
     */
    protected function bindToClass(Request $request, $url, $class)
    {
        $array  = explode('|', $url, 2);
     ...
     * @return CallbackDispatch
     */
    protected function bindToNamespace(Request $request, $url, $namespace)
    {
        $array  = explode('|', $url, 3);
     ...
     */
    protected function bindToController(Request $request, $url, $controller)
    {
        $array  = explode('|', $url, 2);
        $action = !empty($array[0]) ? $array[0] : $this->router->config('default_action');
        $param  = [];
