...
class Request implements ArrayAccess
{
    ...
    public static function __make(App $app)
    {
        ...
        $request->file    = isset($_FILES) ? $_FILES : [];
     ...
     * @return string
     */
    public function baseFile($complete = false)
    {
        if (!$this->baseFile) {
            $url = '';
            if (!$this->isCli()) {
     ...
     * @return string
     */
    public function root($complete = false)
    {
        if (!$this->root) {
            $file = $this->baseFile();
    ...
    public function only(array $name, $data = 'param', $filter = '')
    {
        ...
        foreach ($name as $key => $val) {
            ...
            $item[$key] = $this->filterData(isset($data[$key]) ? $data[$key] : $default, $filter, $key, $default);
        }
     ...
     * @return string
     */
    public function buildToken($name = '__token__', $type = 'md5')
    {
